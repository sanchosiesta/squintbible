          ...
          (= key "y") (do
                        ...
                        (let [now (.now js/Date)]
                          (if (and (.-_lastYKey js/window) (< (- now (.-_lastYKey js/window)) 500))
                            (do (yank-verse!) (set! (.-_lastYKey js/window) nil))
                            (set! (.-_lastYKey js/window) now))))
          ;; g - start of gg sequence (double-tap g = goto top)
          (= key "g") (do
                        (.preventDefault e)
                        (let [now (.now js/Date)]
                          (if (.-_lastGKey js/window)
                            (if (< (- now (.-_lastGKey js/window)) 500)
                              ;; gg double-tap fired → goto top.
                              ;; Set _lastGKey to a fresh timestamp (NOT nil) so a
                              ;; subsequent 'u'/'g' within 500ms still works. Using
                              ;; nil here would permanently lock out 'gu' after 'gg'.
                              (do (goto-top!)
                                  (set! (.-_lastGKey js/window) now))
                              ;; stale g-prefix → restart sequence with a NEW
                              ;; timestamp (NOT 0, which is truthy in squint/CLJS
                              ;; and would cause a permanent lockout).
                              (set! (.-_lastGKey js/window) now))
                            (set! (.-_lastGKey js/window) now))))
          ...
          (= key "u") (if (.-ctrlKey e)
                        ...
                        (when-let [last-g (.-_lastGKey js/window)]
                          ...
                          (when (< (- (.now js/Date) last-g) 500)
                            (search-verse!))
                          ;; reset to nil so next 'g' starts a fresh sequence
                          (set! (.-_lastGKey js/window) nil)))
          ;; Ctrl shortcuts
